...
test('manifest route is France-specific',async()=>{
  ...
  try{
    ...
    assert.equal(m.id,'com.nuvio.calendar.archives.fr');
...
test('collections route exposes French platform parents and hosted Modern images',async()=>{
  ...
  try{
    ...
    assert.match(canal.folders[0].coverImageUrl,/platform-category-card\.svg\?provider=canal-plus&category=series&v=fr100$/);
    assert.match(canal.folders[0].titleLogoUrl,/platform-logo\?provider=canal-plus&type=series&v=fr100$/);
  }finally{delete process.env.NUVIO_NOW_OVERRIDE}
});
...
test('France category-card and hero routes render large platform names even without TMDb credentials',async()=>{
  ...
  try{
    const card=await call('/platform-category-card.svg?provider=canal-plus&category=films&v=fr100');
    assert.equal(card.statusCode,200);assert.match(card.text,/CANAL\+/);assert.match(card.text,/FILMS/);
    const logo=await call('/platform-logo?provider=hbo-max&type=series&v=fr100');
