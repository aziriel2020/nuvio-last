...
const EDGE_CACHE_REV = 'catalog-hardened-v5-bounded-caches';
...
export function platformStaticAssetPath(urlLike) {
  ...
  // collection has a real cinematic card on both Oracle and Oracle.
  ...
  try {
    const upstream = await fetch(src, {
      ...
      headers: {
        ...
        'User-Agent': 'NuvioCalendar/1.4.0 Oracle'
      }
    });
...
async function invokeNodeHandler(request, timeZone) {
  ...
  return new Promise((resolve) => {
    ...
    Promise.resolve(nodeHandler(req, res)).catch((error) => {
      console.error('Oracle native handler error', error);
      if (ended) return;
      ended = true;
      resolve(new Response(JSON.stringify({ error: 'Oracle runtime error' }), {
        status: 502,
        headers: {
