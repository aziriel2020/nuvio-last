...
const ARCHIVE_VOD_PROVIDER = Object.freeze({ slug: 'vod-global', label: 'VOD Mondiale', aliases: [] });
const ARCHIVE_ANIME_PROVIDER = Object.freeze({ slug: 'anime-asia', label: 'Anime Japon + Corée', aliases: [] });
const ARCHIVE_SERIES_PROVIDERS = Object.freeze([ARCHIVE_ANIME_PROVIDER]);
const ARCHIVE_FILM_PROVIDERS = Object.freeze([ARCHIVE_ANIME_PROVIDER, ARCHIVE_VOD_PROVIDER]);
const ARCHIVE_PROVIDER_BY_SLUG = new Map([
  [ARCHIVE_ANIME_PROVIDER.slug, ARCHIVE_ANIME_PROVIDER],
...
const PLATFORM_COLLECTIONS = Object.freeze([
  { provider: ARCHIVE_ANIME_PROVIDER, categories: [{ key: 'series', type: 'series', title: 'Séries' }, { key: 'films', type: 'movie', title: 'Films' }] },
...
const PLATFORM_COLLECTION_ID_OVERRIDES = Object.freeze({
  'anime-asia': 'calendar-archives-global-anime',
  'vod-global': 'calendar-archives-global-vod'
});


const ANIME_COUNTRY_ORIGINS = new Set(['JP', 'KR']);
...
async function buildAnimeAsiaMovieCatalog({ catalog, timeZone, now = new Date(), period = catalog.period, useCache = true }) {
  ...
  const raw = await discoverGlobalAnimeMovieCandidates(window);
...
function archiveSourceFor(type, provider) {
  if (provider.slug === ARCHIVE_ANIME_PROVIDER.slug) return type === 'movie' ? 'tmdb-anime-jpkr' : 'anilist-anime-jpkr';
...
function resolveArchiveCatalog(catalogId, type, now = runtimeNow(), timeZone = DEFAULT_TIMEZONE) {
  ...
  const dynamic = raw.match(/^archives-global-v1-(series|movie)-([a-z0-9-]+)-(today|tomorrow|yesterday|lastweek|nextweek)$/);
  if (dynamic) {
  ...
  const monthly = raw.match(/^archives-global-v1-(series|movie)-([a-z0-9-]+)-(\d{4})-(\d{2})$/);
...
function collectionAddonSource(entry) {
  return {
    provider: 'addon',
    addonId: 'com.nuvio.calendar.archives.global.coexist',
...
function buildArchiveBlueprint(now = runtimeNow(), timeZone = DEFAULT_TIMEZONE, origin = null) {
  ...
  return {
    schema: 'nuvio-calendar-archives-global-blueprint-v1.0.0',
...
const MOVIE_SPECIAL_FILTERS = Object.freeze([
  { label: 'VOD Mondiale', value: 'vod', kind: 'source' },
  { label: 'Multi-plateformes', value: 'multi', kind: 'special' }
]);
...
function filterCombinedMetas(metas, filter, type) {
  ...
  if (filter.value === 'vod') {
    return (metas || []).filter((meta) => (meta._calendarProviders || []).includes('VOD Mondiale') || meta._calendarSource === 'tmdb-vod');
  }
...
function buildManifest(origin, now = runtimeNow(), timeZone = DEFAULT_TIMEZONE) {
  ...
  return {
    id: 'com.nuvio.calendar.archives.global.coexist',
    version: VERSION,
    name: 'Nuvio Global Archives',
    description: 'Nuvio Global Archives : Anime Japon + Corée et VOD Mondiale avec périodes dynamiques et archives mensuelles.',
...
function vodDiscoverParams(window, page) {
  // VOD Mondiale = première sortie Digital (TMDb type 4) quel que soit le pays.
  // Aucun filtre de boutique, watch_region ou region. Les candidats sont ensuite
  // vérifiés via /movie/{id}/release_dates afin de retenir uniquement la toute
  // première date Digital connue mondialement.
  return {
    language: getConfig().language,
    page,
    include_adult: false,
...
async function buildVodCatalog({ catalog, timeZone, now = new Date(), period = catalog.period, useCache = true }) {
  ...
  const stats = emptyStats({ label: 'VOD Mondiale · Digital Releases', ids: [] }, { ...catalog, period, providerSlug: 'vod-global' }, window, timeZone);
  if (window.empty) {
    const result = { metas: [], stats };
    return useCache ? catalogCache.set(key, result, CATALOG_TTL_MS) : result;
  }
  ...
  const settled = await mapLimitSettled(raw, ENRICH_CONCURRENCY, async (candidate) => {
    ...
    return movieVodDetailsToMeta(details, 'VOD Mondiale · Première sortie digitale', window);
  });
...
function combinedLeafCatalogs(catalogOrType) {
  ...
  if (section === 'archive-month') {
    ...
    return [
      ...seriesStreaming,
      ...movieStreaming,
      { type: 'movie', name: 'VOD Mondiale', providerSlug: 'vod-global', period: 'today', source: 'tmdb-vod', explore: false, section: 'films' },
...
async function buildCatalog(options) {
  ...
  if (source === 'anilist-anime-jpkr') return buildAnimeAsiaSeriesCatalog(options);
  if (source === 'tmdb-anime-jpkr') return buildAnimeAsiaMovieCatalog(options);
...
function landing(origin, timeZone = DEFAULT_TIMEZONE) {
  ...
  return `<!doctype html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Nuvio VOD Mondiale</title><style>body{margin:0;background:#050a12;color:#f8fbff;font:16px system-ui,sans-serif;display:grid;place-items:center;min-height:100vh}.card{max-width:980px;margin:24px;padding:32px;border:1px solid #17365e;border-radius:24px;background:linear-gradient(145deg,#08111f,#0a2344)}h1{margin-top:0}.pill{display:inline-block;background:#0b67c2;padding:8px 14px;border-radius:999px;font-weight:800}code{display:block;overflow-wrap:anywhere;background:#030912;padding:14px;border-radius:12px;margin:14px 0}a{color:#58c7ff}.muted{color:#a9bdd4}.ok{color:#7ee787}.bad{color:#ff7b72}</style></head><body><main class="card"><span class="pill">ARCHIVES MODERN SHIELD · PLATEFORMES</span><h1>Nuvio VOD Mondiale ${VERSION}</h1><p>Hiérarchie native : <b>VOD Mondiale → Films → périodes dynamiques → mois + année → contenus</b>.</p><p>Parent unique : <b>🌍 VOD Mondiale</b>, Films uniquement. Un film est classé à sa <b>toute première date Digital mondiale</b> connue dans TMDb (type 4), tous pays confondus, sans filtre achat/location ni boutique.</p><p>Chaque dossier commence par <b>Aujourd’hui, Demain, Hier, Semaine passée, La semaine suivante</b>, puis les mois. Les cinq périodes se recalculent chaque jour selon le fuseau du spectateur. Aujourd’hui : <b>${currentMonth} ${year}</b> est le premier mois courant; quand septembre arrive, <b>Septembre ${year}</b> apparaît automatiquement sans réimport.</p><p>Visuels : cartes 16:9 Modern vectorielles, backgrounds nets et <b>logos plateforme TMDb en résolution originale, grands et centrés</b>, avec overlay Séries / Films.</p><p>Fuseau spectateur : <b>${timeZone}</b> — marché : <b>GLOBAL</b> — TMDb : <b class="${configured ? 'ok' : 'bad'}">${configured ? 'configuré' : 'clé manquante'}</b></p><p>Manifest :</p><code>${manifest}</code><p><a href="${manifest}">manifest.json</a> · <a href="${collectionsImport}">JSON Collections importable</a> · <a href="${blueprint}">blueprint</a> · <a href="${origin}/health">health</a></p><p class="muted">Pour avoir les vraies images sur la Shield, importe le JSON depuis l’URL déployée <b>/nuvio-collections.json</b> : il injecte automatiquement les URLs du déploiement dans les cartes et les backdrops.</p></main></body></html>`;
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    return json(res, 502, { error: 'Impossible de charger la VOD mondiale pour le moment.' }, 'no-store');
  }
};
