...
function globalDigitalReleaseDates(details) {
  const dates = [];
  for (const country of (details?.release_dates?.results || [])) {
    const countryCode = String(country?.iso_3166_1 || '').trim().toUpperCase() || null;
    for (const entry of (country?.release_dates || [])) {
      if (Number(entry?.type) !== 4) continue;
      const date = normalizeIsoDate(entry?.release_date);
      if (!date) continue;
      dates.push({ ...entry, date, country: countryCode });
    }
  }
  return dates.sort((a, b) => a.date.localeCompare(b.date) || String(a.country || '').localeCompare(String(b.country || '')));
}

function selectDigitalRelease(details, window) {
  const dates = globalDigitalReleaseDates(details);
  if (!dates.length) return { release: null, reason: 'date-unknown' };
  const first = dates[0];
  if (isDateInWindow(first.date, window)) return { release: first, reason: null };
  if (first.date < window.today) return { release: null, reason: 'past' };
...
function baseMeta(details, type, releaseDate, releaseInfo) {
  ...
  return {
    ...
    country: Array.isArray(details?.production_countries) && details.production_countries.length
      ? details.production_countries.map((entry) => entry?.name || entry?.iso_3166_1).filter(Boolean).join(', ')
      : null,
...
function movieVodDetailsToMeta(details, providerLabel, window) {
  ...
  const meta = baseMeta(
    ...
    'movie',
    event.viewerDate,
    `Digital Release • ${humanDate(event.viewerDate, DEFAULT_TIMEZONE, window.today)}`
  );
  meta.description = [`${providerLabel} • Première date Digital mondiale TMDb${selected.release.country ? ` • ${selected.release.country}` : ''}`, meta.description].filter(Boolean).join('\n\n');
  meta._eventMode = event.eventMode;
  return { meta, reason: null, event };
}
