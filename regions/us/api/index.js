...
const SOURCE_VERSION = 'calendar-archives-v1.3.1-modern-shield';
const VISUAL_REV = 'coex-us131-cinematic';
...
function buildGenreFolder(entries, origin, genre, type) {
  ...
  return {
    id: `genres-us-${isMovie ? 'movie' : 'series'}-folder-${genre.id}`,
    title: genre.name,
...
function buildGenreCollectionByType(entries, origin = null, type = 'movie') {
  ...
  return {
    id: isMovie ? 'calendar-archives-us-genres' : 'calendar-archives-us-genres-series',
...
function genreFolderArtSvg(params = {}) {
  ...
  const typeLabel = typeToken === 'series' ? 'SERIES' : 'MOVIES';
...
function collectionAddonSource(entry) {
  return {
    provider: 'addon',
    addonId: 'com.nuvio.calendar.archives.us.coexist',
