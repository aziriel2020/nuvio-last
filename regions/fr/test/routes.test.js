...
test('France category-card and hero routes render large platform names even without TMDb credentials',async()=>{
  ...
  try{
    const card=await call('/platform-category-card.svg?provider=canal-plus&category=films&v=coex-fr131-cinematic');
    assert.equal(card.statusCode,200);assert.match(card.text,/CANAL\+/);assert.match(card.text,/FILMS/);
    const logo=await call('/platform-logo?provider=hbo-max&type=series&v=coex-fr131-cinematic');
