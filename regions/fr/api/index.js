...
const SOURCE_VERSION = 'calendar-archives-fr-v1.2.0-modern-shield';
const VISUAL_REV = 'coex-fr120-cinematic';
...
function resolveArchiveCatalog(catalogId, type, now = runtimeNow(), timeZone = DEFAULT_TIMEZONE) {
  ...
  const genre = raw.match(/^genres-fr-(series|movie)-(\d+)$/);
  if (genre) {
    const expectedType = genre[1] === 'movie' ? 'movie' : 'series';
    if (type !== expectedType) return null;
    const list = expectedType === 'movie' ? TMDB_MOVIE_GENRES : TMDB_TV_GENRES;
    const match = list.find((entry) => entry.id === Number(genre[2]));
    return match ? genreDescriptor(expectedType, match) : null;
  }
