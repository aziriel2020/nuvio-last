...
function baseMeta(details, type, releaseDate, releaseInfo) {
  ...
  return {
    ...
    country: details?.production_countries?.[0]?.name || details?.origin_country?.[0] || null,
...
function movieDetailsToMeta(details, providerLabel, window) {
  ...
  meta.description = [`${providerLabel} Türkiye • Première digitale`, meta.description].filter(Boolean).join('\n\n');
...
function movieVodDetailsToMeta(details, providerLabel, window) {
  ...
  meta.description = [`${providerLabel} • Date Digital Türkiye vérifiée`, meta.description].filter(Boolean).join('\n\n');
  meta._eventMode = event.eventMode;
  return { meta, reason: null, event };
}
...
function seriesDetailsToMeta(details, providerLabel, window) {
  ...
  meta.description = [`${providerLabel} Türkiye • Date streaming`, meta.description].filter(Boolean).join('\n\n');
  meta._eventMode = event.eventMode;
