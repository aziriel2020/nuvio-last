...
const DESKTOP_FONT_CACHE = { font: null };

function desktopFont() {
  if (DESKTOP_FONT_CACHE.font) return DESKTOP_FONT_CACHE.font;
  const fontPath = require.resolve('dejavu-fonts-ttf/ttf/DejaVuSans-Bold.ttf');
  DESKTOP_FONT_CACHE.font = opentype.loadSync(fontPath);
  return DESKTOP_FONT_CACHE.font;
}

function normalizedDesktopText(value) {
  return String(value || '').replace(/\s+/g, ' ').trim();
}

function fitDesktopText(text, maxWidth, preferredSize, minSize = 44) {
  const value = normalizedDesktopText(text);
  if (!value) return { text: '', size: preferredSize };
  const font = desktopFont();
  let size = preferredSize;
  while (size > minSize && font.getAdvanceWidth(value, size, { kerning: true }) > maxWidth) {
    size -= 2;
  }
  if (font.getAdvanceWidth(value, size, { kerning: true }) <= maxWidth) {
  ...
  while (clipped.length > 3) {
    ...
    if (font.getAdvanceWidth(candidate, size, { kerning: true }) <= maxWidth) {
...
function desktopPathText(text, x, baselineY, maxWidth, preferredSize, options = {}) {
  const fitted = fitDesktopText(text, maxWidth, preferredSize, options.minSize || 24);
...
function desktopCenteredPathText(text, centerX, baselineY, maxWidth, preferredSize, options = {}) {
  const fitted = fitDesktopText(text, maxWidth, preferredSize, options.minSize || 20);
  if (!fitted.text) return '';
  const font = desktopFont();
...
function desktopOverlaySvg(type = 'series', accent = '#38bdf8', options = {}) {
  ...
  const title = normalizedDesktopText(options.title || (movie ? 'Film' : 'Série'));
  const subtitle = normalizedDesktopText(options.subtitle || '');
  const providerLabel = normalizedDesktopText(options.providerLabel || '').replace(/^[^\p{L}\p{N}]+/u, '').toUpperCase();
  ...
  return Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="1600" height="900" viewBox="0 0 1600 900">
    <defs>
      <linearGradient id="shade" x1="0" y1="0" x2="1" y2="0">
        <stop offset="0%" stop-color="#02040a" stop-opacity=".46"/>
        <stop offset="42%" stop-color="#02040a" stop-opacity=".05"/>
        <stop offset="100%" stop-color="#02040a" stop-opacity=".25"/>
      </linearGradient>
      <linearGradient id="bottom" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#041021" stop-opacity="0"/>
        <stop offset="30%" stop-color="#072650" stop-opacity=".35"/>
...
async function desktopCinematicCardBuffer(sourceBuffer, options = {}) {
  ...
  return sharp(sourceBuffer)
    ...
    .modulate({ brightness: 0.96, saturation: 1.07 })
...
async function handleDesktopFolderCard(res, url) {
  ...
  try {
    const cinematic = await platformCinematicBackdrop(providerSlug, type);
    const source = cinematic?.buffer || fs.readFileSync(platformArtPath(providerSlug, 'card'));
...
const ARCHIVE_ID_PREFIX = 'archives-tr-v1';
const ARCHIVE_PREWIRE_FUTURE_YEARS = 4;
const ARCHIVE_MONTHS_FR = Object.freeze([
  'Janvier', 'Février', 'Mars', 'Avril', 'Mai', 'Juin',
  'Juillet', 'Août', 'Septembre', 'Octobre', 'Novembre', 'Décembre'
]);
const ARCHIVE_VOD_PROVIDER = Object.freeze({ slug: 'vod-tr', label: 'VOD Türkiye', aliases: [] });
const ARCHIVE_ALL_PROVIDER = Object.freeze({
  slug: 'turkiye-takvim',
  label: 'Türkiye Takvim',
  aliases: [],
  aggregate: true,
  monetizationTypes: ['flatrate', 'free', 'ads']
});
const ARCHIVE_SERIES_PROVIDERS = Object.freeze([
  ...PROVIDERS,
...
const ARCHIVE_PROVIDER_BY_SLUG = new Map([
  ...
  [ARCHIVE_ALL_PROVIDER.slug, ARCHIVE_ALL_PROVIDER],
...
// Native Nuvio hierarchy used by the Türkiye edition:
...
function genreCatalogBaseId(type, genreId) {
  return `genres-tr-${type === 'movie' ? 'movie' : 'series'}-${genreId}`;
}
...
function buildGenreFolder(entries, origin, genre, type) {
  ...
  return {
    id: `genres-tr-${isMovie ? 'movie' : 'series'}-folder-${genre.id}`,
    title: genre.name,
...
function buildGenreCollectionByType(entries, origin = null, type = 'movie') {
  ...
  return {
    ...
    id: isMovie ? 'calendar-archives-tr-genres' : 'calendar-archives-tr-genres-series',
...
function resolveArchiveCatalog(catalogId, type, now = runtimeNow(), timeZone = DEFAULT_TIMEZONE) {
  ...
  const genreMonthly = raw.match(/^genres-tr-(series|movie)-(\d+)-(\d{4})-(\d{2})$/);
  if (genreMonthly) {
  ...
  const genreDynamic = raw.match(/^genres-tr-(series|movie)-(\d+)(?:-(today|tomorrow|yesterday|lastweek|nextweek))?$/);
  if (genreDynamic) {
  ...
  const dynamic = raw.match(/^archives-tr-v1-(series|movie)-([a-z0-9-]+)-(today|tomorrow|yesterday|lastweek|nextweek)$/);
  if (dynamic) {
  ...
  const monthly = raw.match(/^archives-tr-v1-(series|movie)-([a-z0-9-]+)-(\d{4})-(\d{2})$/);
...
function collectionAddonSource(entry) {
  return {
    provider: 'addon',
    addonId: 'com.nuvio.calendar.archives.tr.coexist',
...
function platformCollectionId(providerSlug) {
  return `calendar-archives-tr-${providerSlug}`;
}
...
function buildArchiveBlueprint(now = runtimeNow(), timeZone = DEFAULT_TIMEZONE, origin = null) {
  ...
  return {
    schema: 'nuvio-calendar-archives-tr-blueprint-v1.0.2',
...
const TVUSA_FILTERS = Object.freeze([
  { label: 'TV Türkiye • Prioritaire', value: 'tv-tr', kind: 'source' },
  { label: 'TV Türkiye • Tout', value: 'tv-tr-all', kind: 'source' }
]);
...
const MOVIE_SPECIAL_FILTERS = Object.freeze([
  { label: 'VOD Türkiye', value: 'vod', kind: 'source' },
  { label: 'Multi-plateformes', value: 'multi', kind: 'special' }
]);
...
function filterCombinedMetas(metas, filter, type) {
  ...
  if (filter.value === 'vod') {
    return (metas || []).filter((meta) => (meta._calendarProviders || []).includes('VOD Türkiye') || meta._calendarSource === 'tmdb-vod');
  }
  if (type !== 'series') return metas || [];
  if (filter.value === 'tv-tr' || filter.value === 'tv-tr-all') return (metas || []).filter((meta) => meta._calendarSource === 'tvmaze-broadcast');
...
function calendarSourceLabel(source) {
  if (source === 'tvmaze-broadcast') return 'TV Türkiye';
...
function desktopContentCardUrl(origin, meta, catalog, sourceOverride = null) {
  ...
  url.searchParams.set('title', compactCardText(meta?.name || '', 68));
  url.searchParams.set('append', calendarAppend(meta, catalog));
  url.searchParams.set('label', compactCardText(meta?._calendarProvider || catalog?.cardProvider || platformCollectionTitle(providerSlug), 30));
...
function buildManifest(origin, now = runtimeNow(), timeZone = DEFAULT_TIMEZONE) {
  ...
  return {
    id: 'com.nuvio.calendar.archives.tr.coexist',
    version: VERSION,
    name: 'Nuvio Calendar Türkiye',
    description: 'Calendrier streaming Türkiye : Netflix, Prime Video, Disney+, Max, Apple TV+, MUBI, Exxen, GAİN, tabii, TOD, puhutv, TV+, Tivibu, D-Smart GO, S Sport Plus, calendrier global et VOD Türkiye.',
...
async function resolveProvider(providerSlug, type) {
  const directory = await providerDirectory(type);
  if (providerSlug === ARCHIVE_ALL_PROVIDER.slug) {
    const resolved = PROVIDERS.map((definition) => resolveProviderFromDirectory(definition, directory));
    return {
      ...ARCHIVE_ALL_PROVIDER,
      ids: [...new Set(resolved.flatMap((entry) => entry.ids || []))],
      matchedNames: [...new Set(resolved.flatMap((entry) => entry.matchedNames || []))],
      logoPaths: []
    };
  }
...
function platformProviderDefinition(providerSlug) {
  ...
  if (providerSlug === ARCHIVE_ALL_PROVIDER.slug) return ARCHIVE_ALL_PROVIDER;
...
async function platformLogoAsset(providerSlug, type = 'movie') {
  if (providerSlug === ARCHIVE_VOD_PROVIDER.slug || providerSlug === ARCHIVE_ALL_PROVIDER.slug) return null;
...
function theatricalMovieToMeta(details, window, mode) {
  ...
  const prefix = mode === 'nowplaying' ? 'Actuellement au cinéma Türkiye' : 'Sortie cinéma Türkiye à venir';
  const meta = baseMeta(details, 'movie', release.date, `${prefix} • ${humanCalendarDate(release.date)}`);
  meta.description = [prefix, `Date cinéma Türkiye : ${humanCalendarDate(release.date)}`, meta.description].filter(Boolean).join('\n\n');
...
function archiveEpisodeToMeta(details, episode, providerLabel, window) {
  ...
  meta.description = [
    `${providerLabel} Türkiye • épisode diffusé en ${String(window.start).slice(0, 4)}`,
...
async function buildVodCatalog({ catalog, timeZone, now = new Date(), period = catalog.period, useCache = true }) {
  ...
  const stats = emptyStats({ label: 'VOD Türkiye · Sorties digitales', ids: [] }, { ...catalog, period, providerSlug: 'vod-tr' }, window, timeZone);
  if (window.empty) {
    const result = { metas: [], stats };
    return useCache ? catalogCache.set(key, result, CATALOG_TTL_MS) : result;
  }
  ...
  const settled = await mapLimitSettled(raw, ENRICH_CONCURRENCY, async (candidate) => {
    ...
    return movieVodDetailsToMeta(details, 'VOD Türkiye · Sortie digitale', window);
  });
...
async function tvmazeScheduleDate(sourceDate) {
  ...
  if (cached) return cached;
  const payload = await tvmazeFetch('/schedule', { country: 'TR', date: sourceDate });
...
function tvmazeBroadcastToMeta(episode, timeZone, window, now = new Date()) {
  ...
  const description = [
    episodeTitle,
    sourceLabel ? `Diffusion Türkiye : ${sourceLabel}` : null,
    'Horaires : TVmaze',
    summary
  ].filter(Boolean).join('\n\n');
...
async function buildTvBroadcastCatalog({ catalog, timeZone, now = new Date(), period = catalog.period, useCache = true }) {
  const window = dateWindow(period, now, timeZone);
  const key = catalogCacheKey({
    providerSlug: catalog?.includeLowSignal ? 'tv-tr-all' : 'tv-tr-clean',
    type: 'series',
    period,
    timeZone,
    today: window.today,
...
function tvmazeStreamingEpisodeToMeta(episode, details, provider, timeZone, window) {
  ...
  const timingNote = event.viewerTime
    ? `${provider.label} Türkiye • heure de mise en ligne TVmaze convertie en ${timeZone}`
    : `${provider.label} Türkiye • date streaming officielle, heure non annoncée`;
  meta.description = [
...
async function buildStreamingSeriesCatalog({ catalog, timeZone, now = new Date(), period = catalog.period, useCache = true }) {
  ...
  const fallbackSettled = await mapLimitSettled(tmdbCandidates, ENRICH_CONCURRENCY, async (candidate) => {
    ...
    converted.meta.description = [
      `${provider.label} Türkiye • nouvel épisode confirmé TMDb`,
...
function combinedLeafCatalogs(catalogOrType) {
  ...
  if (section === 'archive-month') {
    ...
    return [
      ...seriesStreaming,
      ...movieStreaming,
      { type: 'movie', name: 'VOD Türkiye', providerSlug: 'vod-tr', period: 'today', source: 'tmdb-vod', explore: false, section: 'films' },
      ...
      { type: 'series', name: 'TV Türkiye', providerSlug: 'tv-tr', period: 'today', source: 'tvmaze-broadcast', includeLowSignal: false, explore: false, section: 'tvusa' }
    ];
  }

  if (section === 'tvusa') return [{ type: 'series', name: 'TV Türkiye', providerSlug: 'tv-tr', period: 'today', source: 'tvmaze-broadcast', includeLowSignal: false, explore: false, section: 'tvusa' }];
...
function mergeCombinedMetas(results, type, period = 'week') {
  ...
  for (const meta of values) {
    ...
    meta._calendarProvider = providers.join(' + ') || 'Calendar Türkiye';
    if (meta._calendarSource === 'tmdb-streaming' || meta._calendarSource === 'tmdb-vod' || meta._calendarSource === 'tmdb-digital') {
      ...
      meta.description = [
        `Plateforme${providers.length > 1 ? 's' : ''} Türkiye : ${providers.join(' • ')}`,
        body
      ].filter(Boolean).join('\n\n');
    }
  }
...
async function handleHealth(req, res) {
  ...
  if (!configured) {
    ...
    return json(res, 503, {
      ...
      mode: 'calendar-archives-tr-modern-shield-v1.0',
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (error?.code === 'TMDB_HTTP_ERROR') {
      return json(res, 502, {
        error: 'Impossible de charger les nouvelles sorties Türkiye pour le moment.',
        tmdbStatus: error.status || null,
    ...
    return json(res, 502, { error: 'Impossible de charger le calendrier Türkiye pour le moment.' }, 'no-store');
  }
};
